...
#include "controlper/ControlNode.hpp"
...
: Node("control_node"), tf_buffer_(), tf_listener_(tf_buffer_), lin_pid_(0.0, 1.0, 0.0, 0.7), rot_pid_(0.0, 1.0, 0.3, 1.0)
{
...
ControlNode::seguir()
{
  ...
  //if ((this->now() - rclcpp::Time(std::chrono::system_clock::to_time_t(camera2target.stamp_))).seconds() < 0.1){
  //  detected_=true;
  //} else {
  //  detected_=false;
  //}
  ...
  if (detected_){
    ...
    message_.linear.x = std::clamp(lin_pid_.get_output(dist), -0.5, 0.5);
